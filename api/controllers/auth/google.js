...
export default {
  ...
  async signin (req, res) {
    ...
    if (userData) {
      // 使用者登入
      let { social: sc } = userData
      let scKey = Object.keys(social)
      console.log(`${JSON.stringify(sc)} / ${JSON.stringify(social)}`)

      for (let i = 0, len = scKey.length; i < len; i++) {
        let k = scKey[i]
        if (!_.eq(social[k], sc[k])) return res.json({ status: 400, message: '帳號或密碼錯誤' })
      }
    } else {
      // 註冊
